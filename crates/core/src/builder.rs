...
mod tests {
    ...
    #[test]
    fn ast_limit() {
        let result = parse_to_ast("\\lim_{x \\to 0} x").unwrap();
        assert_eq!(
            result,
            Expr::Limit(
                Box::new(Expr::Identifier("x".to_string())),
    ...
    #[test]
    fn ast_sum() {
        let result = parse_to_ast("\\sum_{i=1}^{n} i").unwrap();
        assert_eq!(
            result,
            Expr::Sum(
    ...
    #[test]
    fn ast_product() {
        let result = parse_to_ast("\\prod_{i=1}^{n} i").unwrap();
        assert_eq!(
            result,
            Expr::Product(
                Box::new(Expr::Identifier("i".to_string())),
    ...
    #[test]
    fn ast_integral() {
        let result = parse_to_ast("\\int_{0}^{1} x").unwrap();
        assert_eq!(
            result,
            Expr::Integral(
                Box::new(Expr::Number(0.0)),
